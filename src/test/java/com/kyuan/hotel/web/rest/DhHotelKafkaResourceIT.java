...
class DhHotelKafkaResourceIT {
    ...
    void setup() {
        ...
        DhHotelKafkaResource kafkaResource = new DhHotelKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dh-hotel-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dh-hotel-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
